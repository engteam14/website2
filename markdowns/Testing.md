...
Due to the limitations presented during testing, such as issues where testing couldn’t be done if it required testing, and there being pre-existing code that isn’t implemented and couldn’t be removed (such as methods within classes which implement interfaces)  we were only able to achieve **63.1% code coverage**. However, the portion of code which is untested we have verified the functionality of through manual testing, and the portion of the code which is unimplemented doesn’t need to be tested.
